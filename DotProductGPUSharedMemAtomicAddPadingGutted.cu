// nvcc DotProductGPUSharedMemAtomicAddPadingGutted.cu -o temp
...
// N % blockDim.x <- gives you number of extra spaces leftover within your last block.
...
void SetUpCudaDevices(int vectorSize)
{
	...
	GridSize.x = getNumberBlocks(BlockSize.x, N); //Makes enough blocks to deal with the whole vector.
...
{					
	...
	memset(A_CPU, 0, vectorSize);
	memset(B_CPU, 0, vectorSize);
