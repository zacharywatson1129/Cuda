...
/*__global__ void setColor(float *pixels, int index)
{
	if (currColor == RED)
	{
		pixels[index] = 1.0;
...
__global__ void colorPixels(float *pixels, float xMin, float yMin, float dx, float dy, float a, float b) 
{
	...
	if(0.99999 < escapeValue)
	{
		...
		pixels[id+1] = 0.5; //Setting the green
		pixels[id+2] = 0.5; //Setting the blue
	}
	else
	{
...
void adjustSeed()
{
	...
	float temp2 = cos(time);
	float temp = ((float)rand()/(float)RAND_MAX)*2.0 - 1.0; // Get random number between -1 at 1.
...
int main(int argc, char** argv)
{
	...
	//start = (end.tv_sec * 1000000 + end.tv_usec) - (start.tv_sec * 1000000 + start.tv_usec);
