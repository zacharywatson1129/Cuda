...
__global__ void DotProductGPU(float *a, float *b, float *c, int n)
{
    ...
    __syncthreads(); // We should be finished multiplying before we continue.

    // Fold starts for every thread with how many threads the block has.
    int fold = blockDim.x;
    while (fold / 2 >= 1) // If fold == 2, we run one last time.
    {
        ...
        // This if breaks the code and I am not sure why.
        if (id < fold / 2)
        {
            c[id] += c[id + fold/2];
        }
