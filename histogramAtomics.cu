...
__global__ void fillHistogramGPU(float *randomNumbers, int *hist)
{
	...
	//****************************************
	__shared__ float tempHisto[BLOCK_SIZE];
	tempHisto[threadIdx.x] = 0; // Ensure every position within the shared memory is 0, or the result might be wrong.
	__syncthreads();
	...
	for(i = id; i < NUMBER_OF_RANDOM_NUMBERS; i += jump)
	{
		...
		// Purpose of this loop is to find out what the correct bin is
		// to increment.
		while(done == 0)
		{
	...
	__syncthreads();
	atomicAdd(&(hist[threadIdx.x]), tempHisto[threadIdx.x] );
}
