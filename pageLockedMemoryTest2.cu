...
{					
	...
	cudaHostAlloc((void**)&PageLockedNumbersOnCPU, 
                    SIZE*sizeof(*PageLockedNumbersOnCPU),
...
void copyPageLockedMemoryDown()
{
	for(int i = 0; i < NUMBER_OF_COPIES; i++)
	{
		cudaMemcpy(PageLockedNumbersOnCPU, NumbersOnGPU, SIZE*sizeof(float), cudaMemcpyHostToDevice);
		myCudaErrorCheck(__FILE__, __LINE__);
	}
}
